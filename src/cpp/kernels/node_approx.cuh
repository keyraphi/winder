...
__device__ __forceinline__ auto
computeZeroOrderContribution(const Vec3_f16 &coeff, const Vec3_f16 &r_hat) -> float {
  half2 c_xy = __halves2half2(coeff.x, coeff.y);
  half2 c_z = __halves2half2(coeff.z, __float2half(0.0f));
...
__device__ __forceinline__ auto
computeFirstOrderContribution(const Mat3x3_f16 &C, const Vec3_f16 &r_hat) -> float {
  // diagonal part
  half trace_C = __hadd(__hadd(C.data[0], C.data[4]), C.data[8]);
  ...
  half2 r2_xy = __halves2half2(__hmul(r_hat.x, r_hat.x), __hmul(r_hat.y, r_hat.y));
  half2 r2_z_xy = __halves2half2(__hmul(r_hat.z, r_hat.z), __hmul(r_hat.x, r_hat.y));
  half2 r2_xz_yz = __halves2half2(__hmul(r_hat.x, r_hat.z), __hmul(r_hat.y, r_hat.z));
...
computeSecondOrderContribution(const Tensor3_f16_compressed &C,
  ...
  // V dot r_hat
  half v_dot_r = __hadd(__hadd(__hmul(vx, r_hat.x), __hmul(vy, r_hat.y)), __hmul(vz, r_hat.z));
  ...
  // --- YOUR EXACT UNROLLED CHUNKS (safely processing r_hat bounds) ---
  half2 accumulator = __hmul2(__halves2half2(C.data[0], C.data[1]), __halves2half2(__hmul(r_xx, r_hat.x), __hmul(r_xx, r_hat.y)));
  accumulator = __hfma2(__halves2half2(C.data[2], C.data[3]), __halves2half2(__hmul(r_xx, r_hat.z), __hmul(__hmul(r_xy, r_hat.x), two)), accumulator);
  accumulator = __hfma2(__halves2half2(C.data[4], C.data[5]), __halves2half2(__hmul(__hmul(r_xy, r_hat.y), two), __hmul(__hmul(r_xz, r_hat.y), two)), accumulator);
  accumulator = __hfma2(__halves2half2(C.data[6], C.data[7]), __halves2half2(__hmul(__hmul(r_xz, r_hat.x), two), __hmul(__hmul(r_yz, r_hat.x), two)), accumulator);
  accumulator = __hfma2(__halves2half2(C.data[8], C.data[9]), __halves2half2(__hmul(__hmul(r_xz, r_hat.z), two), __hmul(r_yy, r_hat.x)), accumulator);
  accumulator = __hfma2(__halves2half2(C.data[10], C.data[11]), __halves2half2(__hmul(r_yy, r_hat.y), __hmul(r_yy, r_hat.z)), accumulator);
  accumulator = __hfma2(__halves2half2(C.data[12], C.data[13]), __halves2half2(__hmul(__hmul(r_yz, r_hat.x), two), __hmul(__hmul(r_yz, r_hat.y), two)), accumulator);
  accumulator = __hfma2(__halves2half2(C.data[14], C.data[15]), __halves2half2(__hmul(__hmul(r_yz, r_hat.z), two), __hmul(r_zz, r_hat.x)), accumulator);
  accumulator = __hfma2(__halves2half2(C.data[16], C.data[17]), __halves2half2(__hmul(r_zz, r_hat.y), __hmul(r_zz, r_hat.z)), accumulator);
...
__device__ __forceinline__ auto compute_node_approximation(
  ...
  // Create a unit vector. Components are <= 1.0, making them perfectly safe for f16 math!
  Vec3_f16 r_hat_f16 = Vec3_f16::from_float(r * inv_norm_r);

  // Keep the massive/tiny scale factors strictly in float32 variables
  ...
  // Compute contractions using the unit vector, then scale via float32 at the end
  result += computeZeroOrderContribution(zero_order_coeff, r_hat_f16) * factor_zero;
  result += computeFirstOrderContribution(first_order_coeff, r_hat_f16) * factor_first;
  result += computeSecondOrderContribution(second_order_coeff, r_hat_f16) * factor_second;
