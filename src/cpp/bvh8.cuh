...
__host__ __device__ auto BVH8Node::get_parent_idx() const -> uint32_t {
...
__host__ __device__ void BVH8Node::set_parent_idx(uint32_t idx) {
  auto *uint32_data = reinterpret_cast<uint32_t *>(tailor_data);
  uint32_data[0] = idx;
}

__host__ __device__ inline auto BVH8Node::get_shared_scale_factor() const
    -> float {
...
BVH8Node::set_tailor_coefficients(const Vec3 &zero_order,
                                  const Mat3x3 &first_order,
  ...
  float shared_scale_factor = __int_as_float(shared_scale_factor_bits);
...
BVH8Node::get_tailor_zero_order(const float shared_scale_factor) const
    -> Vec3_bf16 {
...
__host__ __device__ inline auto
BVH8Node::get_tailor_first_order(const float shared_scale_factor) const
    -> Mat3x3_bf16 {
  const auto *uint32_data = reinterpret_cast<const uint32_t *>(tailor_data);
...
__host__ __device__ inline auto
BVH8Node::get_tailor_second_order(const float shared_scale_factor) const
